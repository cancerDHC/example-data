...
def codeable_concept(text, system, code):
    ...
    return ccdh.CodeableConcept(text=text, coding=ccdh.Coding(system=system, code=code))
...
def create_body_site(site_name):
    ...
    site_mappings = {"Larynx, NOS": ccdh.EnumCRDCHBodySiteSite.Larynx.text}

    # Map values if needed. Otherwise, pass them through unmapped.
    if site_name in site_mappings:
        return ccdh.BodySite(
            site=codeable_concept(site_name, "GDC", site_mappings[site_name])
        )
...
def test_create_body_site():
    ...
    assert (
        body_site_larynx.site.coding[0].code == ccdh.EnumCRDCHBodySiteSite.Larynx.text
    )

    assert type(body_site_larynx_nos) is ccdh.BodySite
    assert (
        body_site_larynx_nos.site.coding[0].code
        == ccdh.EnumCRDCHBodySiteSite.Larynx.text
    )
    ...
    if value in stage_mappings:
        return ccdh.CancerStageObservation(
            ...
            value_codeable_concept=codeable_concept(
                value, "GDC", stage_mappings[value]
            ),
        )
...
def create_stage_from_gdc(diagnosis):
    ...
    if diagnosis.get("ajcc_staging_system_edition") == "7th":
        cancer_stage_method_type = codeable_concept(
            "7th", "GDC", "AJCC staging system 7th edition"
        )
...
def test_create_stage_from_gdc():
    diag1 = {"ajcc_staging_system_edition": "7th", "ajcc_pathologic_stage": "Stage III"}
...
def transform_sample_to_specimen(sample):
    ...
    specimen.source_material_type = codeable_concept(
        sample.get("sample_type"), "GDC", sample.get("sample_type")
    )
    specimen.general_tissue_morphology = codeable_concept(
        sample.get("tissue_type"), "GDC", sample.get("tissue_type")
    )
    specimen.specific_tissue_morphology = codeable_concept(
        sample.get("tumor_code"), "GDC", sample.get("tumor_code")
    )
    specimen.tumor_status_at_collection = codeable_concept(
        sample.get("tumor_descriptor"), "GDC", sample.get("tumor_descriptor")
    )
    specimen.creation_activity = ccdh.SpecimenCreationActivity(
        date_ended=ccdh.TimePoint(date_time=sample.get("created_datetime"))
    )
    return specimen
...
def transform_diagnosis(diagnosis, case):
    samples = list(
        filter(
            ...
            map(transform_sample_to_specimen, case.get("samples") or []),
        )
    )

    ccdh_diagnosis = ccdh.Diagnosis(
        id=diagnosis.get("diagnosis_id"),
        condition=codeable_concept(
            diagnosis.get("primary_diagnosis"),
            "GDC",
            diagnosis.get("primary_diagnosis"),
        ),
        morphology=codeable_concept(
            diagnosis.get("morphology"), "GDC", diagnosis.get("morphology")
        ),
        ...
        identifier=[
            ccdh.Identifier(
                system="GDC-submitter-id", value=diagnosis.get("submitter_id")
            )
        ],
    )
...
def test_transform_diagnosis():
    ...
    case1 = {
        "aliquot_ids": ["96f09bc8-a194-482c-bd17-baf28739e4f8"],
        "case_id": "422a72e7-fe76-411d-b59e-1f0f0812c3cf",
        "created_datetime": "2018-09-13T13:42:10.444091-05:00",
        ...
        "diagnoses": [
            {
                ...
                "created_datetime": "2018-09-22T00:57:04.451898-05:00",
                ...
                "diagnosis_id": "842d6984-7c03-4ab6-95db-42fa2ea699db",
                ...
                "lymphatic_invasion_present": None,
                "masaoka_stage": None,
                ...
                "perineural_invasion_present": None,
                "peripancreatic_lymph_nodes_positive": None,
                "peripancreatic_lymph_nodes_tested": None,
                ...
                "state": "released",
                "submitter_id": "GENIE-DFCI-010671-11105_diagnosis",
                ...
                "vascular_invasion_present": None,
        ...
        "diagnosis_ids": ["842d6984-7c03-4ab6-95db-42fa2ea699db"],
        "disease_type": "Squamous Cell Neoplasms",
        "id": "422a72e7-fe76-411d-b59e-1f0f0812c3cf",
        "index_date": "Initial Genomic Sequencing",
        "lost_to_followup": None,
        "primary_site": "Larynx",
        "sample_ids": ["6f9eeaa3-8bd1-479c-a0fc-98317eb458dc"],
        "samples": [
            {
                ...
                "composition": None,
                "created_datetime": "2018-09-24T17:56:43.873530-05:00",
                "current_weight": None,
                ...
                "sample_id": "6f9eeaa3-8bd1-479c-a0fc-98317eb458dc",
                "sample_type": "Primary Tumor",
                "sample_type_id": None,
                "shortest_dimension": None,
                "state": "released",
                "submitter_id": "GENIE-DFCI-010671-11105",
        ...
        "state": "released",
        "submitter_aliquot_ids": ["GENIE-DFCI-010671-11105_aliquot"],
        "submitter_diagnosis_ids": ["GENIE-DFCI-010671-11105_diagnosis"],
        "submitter_id": "GENIE-DFCI-010671",
...
def test_transform_gdc_data():
    ...
    as_json_str = json_dumper.dumps(
        {"@graph": diagnoses, "@context": jsonldContextAsDict}
    )
    assert type(as_json_str) is str
    as_json = json.loads(as_json_str)
    assert type(as_json) is dict
